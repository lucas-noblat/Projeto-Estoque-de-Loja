...
void salvarLista(LISTA * l){
        ...
        FILE* arquivo = fopen("arquivoLista.dat", "wb");
        PONT ini = l->inicio;
        if (!arquivo) printf("Erro de alocação!");
        else{

            while (ini!=NULL) {
                if(fwrite(&ini->reg, sizeof(REGISTRO), 1, arquivo))
                    ini = ini->prox;
